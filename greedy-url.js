const getURL = (s) => {
  const regx = /(https|http):\/\/\w+.[^\s]+\/*[^\s]*/g;
  const arr = s.match(regx);
  return arr;
};
const greedyQuery = (s) => {
  const regx = /(https|http):\/\/\w+.[^\s]+\/*[^\s]+=[^\s]+=[^\s]+=[^\s]+/g;
  const arr = s.match(regx);
  return arr;
};
const notSoGreedy = (s) => {
  const regx = /https?:\/\/[^\s]*=(?:[^\s]*=){1,2}[^\s]*/g;
  const arr = s.match(regx);
  return arr;
};
...
[
  "http://example.com/hello?you=something&something=you",
  "http://hummm/how?how=come&same=[123,21]&you=nextperson&id",
  "http://www.example.com/mypage.html?crcat=test&crsource=test&crkw=buy-a-loteasy",
  "http://example.com/path?name=Branch&products=[Journeys,Email,Universal%20Ads]interact",
  "https://192.168.1.123?something=nothing&pro=[23]",
  "https://www.notherExample.com/catalog.asp?itemid=232&template=fresh&crcat=ppc&crsource",
  "http://www.example.com/catalog.asp?itemid=232&template=fresh&crcat=ppc&crsource",
];
const $getURL = [
...
const testCases = [
  {
    description: "getURL(dataSet)",
    run: () => getURL(dataSet),
    expected: $getURL,
  },
  {
    description: "greedyQuery(dataSet)",
    run: () => greedyQuery(dataSet),
    expected: $greedyQuery,
  },
  {
    description: "notSoGreedy(dataSet)",
    run: () => notSoGreedy(dataSet),
    expected: $notSoGreedy,
  },
];

testCases.forEach(({ description, run, expected }, idx) => {
  const result = run();
